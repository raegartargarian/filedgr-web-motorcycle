...
const fmtDate = (date?: string) => (date ? formatDate(date) : "N/A");
...
export async function generateVaultProofPdf(vault: VaultDto) {
  ...
  overviewRows.push([
    "Status",
    vault.status ? getStatusConfig("vault", vault.status).label : "",
  ]);
  if (vault.created_at)
    overviewRows.push(["Registered", fmtDate(vault.created_at)]);
  if (vault.ledger) {
    overviewRows.push(["Network", ledgerName(vault.ledger) || vault.ledger]);
  }
  if (vault.streams)
    overviewRows.push(["Service Streams", `${vault.streams.length}`]);
  ...
  overviewRows.push(["Total Service Records", `${totalRecords}`]);
  ...
  if (vault.tx_hash) {
    ...
    doc.text(
      `Network: ${ledgerName(vault.ledger) || vault.ledger || "Unknown"}`,
      margin + 6,
      y,
    );
    y += 5;
  ...
  // ========================================
  // SERVICE STREAMS & RECORDS
  // ========================================
  if (vault.streams && vault.streams.length > 0) {
    sectionTitle("Complete Service History");
    y += 2;
    ...
    for (const stream of vault.streams) {
      ...
      if (stream.status)
        streamMeta.push(getStatusConfig("stream", stream.status).label);
      if (stream.tx_hash) streamMeta.push("Blockchain verified");
      doc.text(
        streamMeta.join("  |  "),
        margin + 6 + doc.getTextWidth(streamName) + 8,
        y,
      );
      ...
      if (stream.tx_hash) {
        ...
        if (stream.tx_hash) {
          ...
          doc.textWithLink(
            "  [verify]",
            margin + 6 + doc.getTextWidth(`Stream TX: ${stream.tx_hash}`),
            y,
            {
              url: verifyUrl,
            },
      ...
      if (attachments.length === 0) {
        ...
        doc.text("No service records uploaded yet", margin + 6, y);
        y += 8;
      } else {
        for (const att of attachments) {
          ...
          doc.setFont("helvetica", "bold");
          doc.setTextColor(17, 24, 39);
          doc.text(att.name || "Service Record", margin + 10, y);

          // Status badge text
          if (att.status) {
            const statusText = getStatusConfig("attachment", att.status).label;
            const nameWidth = doc.getTextWidth(att.name || "Service Record");
            doc.setFontSize(7);
            doc.setFont("helvetica", "normal");
            doc.setTextColor(21, 128, 61);
          ...
          if (att.file_count != null)
            metaParts.push(
              `${att.file_count} file${att.file_count !== 1 ? "s" : ""}`,
            );
          ...
          if (hasFiles) {
            ...
            for (const file of att.files!) {
              ...
              const displayName =
                fname.length > 40 ? fname.slice(0, 37) + "..." : fname;
              ...
              if (file.hash)
                fileMeta.push(`hash: ${file.hash.slice(0, 12)}...`);
              if (fileMeta.length > 0) {
                doc.text(
                  fileMeta.join(" | "),
                  margin + 10 + doc.getTextWidth(`  ${displayName}`) + 3,
                  y,
                );
              }
              y += 5;
  ...
  if (vault.config) {
    ...
    if (vault.config.is_permissioned != null)
      configRows.push([
        "Access Control",
        vault.config.is_permissioned ? "Permissioned" : "Public",
      ]);
    if (vault.config.is_searchable != null)
      configRows.push([
        "Searchable",
        vault.config.is_searchable ? "Yes" : "No",
      ]);
    if (vault.config.is_ai_ready != null)
      configRows.push([
        "AI Processing",
        vault.config.is_ai_ready ? "Enabled" : "Disabled",
      ]);
    if (vault.vault_permission_type)
      configRows.push([
        "Permission Type",
        vault.vault_permission_type.replace(/_/g, " "),
      ]);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Page ${i} of ${pageCount}`, pageWidth - margin - 22, footerY);
  }
